...
def vis(haxby_dataset,rois):
    ...
    important_roi_indices = [int(unique_labels[roi]) for roi in rois]
    
    # === plot_roi funtion:
    
    # Expression for math_img like: (img == 5) + (img == 10) + (img == 15)
    expr = " + ".join([f"(img == {roi+2000})" for roi in important_roi_indices])
    ...
    roi_names = [aal_labels[roi] for roi in rois]
    return roi_names
    
vis(haxby_dataset,[3,4])       
